...
If Google reports a fetch error or you see **500** on `/sitemap.xml` after deploy: redeploy the latest `main` (static sitemap + `content/posts` tracing in `next.config.ts`), then open the sitemap URL in a browser and confirm you get XML with `<url>` entries, not an error page.

Broader post-launch steps (custom domain env, AdSense, GA4): **`docs/post-launch-checklist.md`**.
